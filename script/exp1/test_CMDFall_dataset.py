...
def get_windows_from_df(file: str) -> tuple:
    ...
    # convert CMDFall label to Fall/Non-Fall
    label_windows = np.isin(label_windows, [8, 9, 10, 11, 15, 16, 19, 20]).astype(int)
    return tr.from_numpy(data_windows), tr.from_numpy(label_windows)
...
def test_single_task(model: nn.Module, list_data_files: list, device: str = 'cpu'):
    ...
    y_true = tr.concat(y_true).to('cpu')
    y_pred = tr.concat(y_pred).to('cpu')
...
if __name__ == '__main__':
    ...
    list_data_files = glob('/home/ducanh/parquet_datasets/CMDFall/inertia/subject_*/*.parquet')
