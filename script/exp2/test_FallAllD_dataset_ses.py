...
def get_windows_from_df(file: str) -> tuple:
    ...
    label_session = label_windows.any().astype(int)

    return tr.from_numpy(data_windows), label_session
...
def test_single_task(model: nn.Module, list_data_files: list, device: str = 'cpu'):
    ...
    with tr.no_grad():
        for file in tqdm(list_data_files, ncols=0):
            ...
            pred = tr.sigmoid(pred.max()).cpu()

            y_true.append(label)
            y_pred.append(pred)

    result = cal_score(np.array(y_true), np.array(y_pred))
    return result
