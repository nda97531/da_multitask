...
from da_multitask.public_datasets.base_classes import ParquetDatasetFormatter, NpyWindowFormatter
from da_multitask.public_datasets.constant import G_TO_MS2
from da_multitask.utils.number_array import interval_intersection
from da_multitask.utils.string_utils import rreplace
from da_multitask.utils.pl_dataframe import resample_numeric_df as pl_resample_numeric_df
...
class CMDFallNpyWindow(NpyWindowFormatter):
    ...
    @staticmethod
    def reformat_df_2_dict(df: pd.DataFrame):
        """
        Change dataset format from DF to dict, details are below

        Args:
            df: a DF, each row is a session, columns are:
                - 'subject': subject ID
        ...
        Returns:
            a dict:
                - key: label
                - value: data array shape [num window, window size, channel]
            subject info is dropped
        """
        list_sub_modal = df.columns.to_list()
        list_sub_modal.remove('subject')
        list_sub_modal.remove('label')
        ...
        # drop unknown label
        known_label_idx = modal_dict['label'] != 0
        modal_dict = {k: v[known_label_idx] for k, v in modal_dict.items()}
        # convert 20 labels to binary label: 0 = non-fall; 1 = fall
        modal_dict['label'][
            np.isin(modal_dict['label'], [8, 9, 10, 11, 15, 16, 19, 20])
        ] = CMDFallNpyWindow.FALL_LABEL
        ...
        for label_val in label_list:
            ...
            for submodal in list_sub_modal:
                class_dict[submodal][label_val] = modal_dict[submodal][idx]
...
if __name__ == '__main__':
    parquet_dir = '/mnt/data_drive/projects/processed_parquet/CMDFall'
    npy_dir_nonfall = '/mnt/data_drive/projects/UCD02 - Multitask fall det/npy_data_seq/D2'
    npy_dir_fall = '/mnt/data_drive/projects/UCD02 - Multitask fall det/npy_data_seq/D1'
    name = 'CMDFall'

    window_size_sec = 4
    step_size_sec = 1

    npy_process = CMDFallNpyWindow(
    ...
    data_df = npy_process.run()
    data_dict = npy_process.reformat_df_2_dict(data_df)['waist_acc']

    for label, data_arr in data_dict.items():
        # add timestamp col; 50Hz => interval 20ms
        # shape from [num window, window size, 3(x,y,z)] to [num window, window size, 4(msec,x,y,z)]
        fake_ts = np.expand_dims(np.stack([np.arange(data_arr.shape[1]) * 20] * len(data_arr)), -1)
